...
- Если используете Windows, попробуйте:  
  ```sh
...
❌ Django не установлен или не распознаётся
Проблема:
При запуске команды django-admin startproject project_name выводится ошибка "command not found".
...
Редактировать
pip install Django
Убедитесь, что работаете внутри виртуального окружения (в терминале перед командой должно быть (venv_name)).
...
Если порт уже занят, попробуйте запустить сервер на другом порту:
sh
Копировать
Редактировать
